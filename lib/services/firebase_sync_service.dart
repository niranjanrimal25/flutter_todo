...
class FirebaseSyncService extends ChangeNotifier {
  ...
  Future<bool> initialize() async {
    ...
    try {
      if (Firebase.apps.isEmpty) {
        // Use explicit options on all platforms so initialization works on web
        // and does not rely solely on native config files. The options file is
      ...
      _initialized = false;
      _lastError = kIsWeb
          ? 'Web sync is not fully configured yet. '
              'Please register a web app in Firebase Console and update '
              'lib/firebase_options.dart with the correct web appId.'
  ...
  Stream<List<CloudTodoRecord>> watchTodos() {
    if (!isSignedIn) return const Stream<List<CloudTodoRecord>>.empty();
